            ...
            if response:
                ...
                if (
                    ...
                    if _delivery_policy == "silent":
                        ...
                        # Short Kanban status replies can be generated by
                        # several adjacent state transitions (completion,
                        # review, block) while saying the same thing. Keep the
                        # first delivery and silence equivalent repeats for ten
                        # minutes in this exact topic. Detailed conclusions are
                        # intentionally outside the helper's size bound.
                        try:
                            from gateway.delivery_ledger import has_recent_semantic_delivery

                            _semantic_repeat = await asyncio.to_thread(
                                has_recent_semantic_delivery,
                                platform=str(
                            ...
                            logger.debug(
                                "semantic Kanban suppression check failed",
                                exc_info=True,
                            )
                            _semantic_repeat = False
                        if _semantic_repeat:
                            logger.info(
                                "[%s] Suppressing semantically repeated Kanban status for %s",
                                self.name,
                                event.source.chat_id,
                            )
                            _delivery_intentionally_silent = True
