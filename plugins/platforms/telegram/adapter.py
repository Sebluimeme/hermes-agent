...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_callback_query(
        ...
        # --- Persistent TODO board pagination (tb:<page>|tb:noop) ---
        if data.startswith("tb:"):
            ...
            if not self._is_callback_user_authorized(
                ...
                return
            await self._handle_todo_board_callback(
                query,
                data,
    ...
    async def _handle_todo_board_callback(
        self,
        query,
        data: str,
        *,
        ...
        """Switch the sole task-table message between pre-rendered full pages."""
        try:
            from hermes_constants import get_hermes_home
            ...
            logger.warning("[%s] TODO board state unavailable: %s", self.name, exc)
        ...
        buttons.append(InlineKeyboardButton(
            f"{page_index + 1}/{len(pages)}", callback_data="tb:noop"
        ))
        ...
        try:
            ...
            logger.warning("[%s] TODO board page edit failed: %s", self.name, exc)
