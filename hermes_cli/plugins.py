...
VALID_HOOKS: Set[str] = {
    ...
    # Pre-completion validator hook. Fired before a Kanban completion mutates
    # durable board state. Unlike lifecycle observers, return values are
    # honored: callbacks must return an explicit Result or mapping decision
    # ({"accepted": True}, {"ok": True}, {"allow": True},
    # {"action": "allow"}); they may project metadata via
    # {"metadata": {...}} or veto via an explicit false decision / action=block.
    # None/True/ambiguous mappings, exceptions, and malformed results fail closed.
    # Kwargs include a typed context plus additive scalar fields:
    #   context: KanbanCompletionValidationContext, task_id, title, body,
