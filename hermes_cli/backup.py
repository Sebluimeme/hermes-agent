...
_BACKUP_POLICY_MODULE = None


def _backup_policy_candidates() -> list[Path]:
    """Return reviewed backup-policy script candidates outside this package."""
    paths: list[Path] = []
    override = os.environ.get("HERMES_BACKUP_POLICY_SCRIPT")
    if override:
        paths.append(Path(override).expanduser())
    hermes_root = get_default_hermes_root()
    paths.append(hermes_root / "scripts" / "hermes_backup_policy.py")
    return paths


def _load_backup_policy_module():
    """Load the reviewed local backup policy script without duplicating it here."""
    global _BACKUP_POLICY_MODULE
    if _BACKUP_POLICY_MODULE is not None:
        return _BACKUP_POLICY_MODULE
    checked: list[str] = []
    for path in _backup_policy_candidates():
        checked.append(str(path))
        if not path.is_file():
            continue
        spec = importlib.util.spec_from_file_location("hermes_backup_policy", path)
        if spec is None or spec.loader is None:
            continue
        module_name = "hermes_backup_policy"
        ...
        try:
            ...
            raise
        _BACKUP_POLICY_MODULE = module
        return module
    raise RuntimeError(
        "Hermes backup policy script not found; checked: " + ", ".join(checked)
    )


def _policy_attr(name: str):
    return getattr(_load_backup_policy_module(), name)


class _PolicySetProxy:
    def __init__(self, attr_name: str):
        self._attr_name = attr_name

    def __contains__(self, item):
        return item in _policy_attr(self._attr_name)

    def __iter__(self):
        return iter(_policy_attr(self._attr_name))


_EXCLUDED_DIRS = _PolicySetProxy("EXCLUDED_DIRS")
_EXCLUDED_SUFFIXES = _PolicySetProxy("EXCLUDED_SUFFIXES")
_EXCLUDED_NAMES = _PolicySetProxy("EXCLUDED_NAMES")
...
def _iter_external_files(base: Path) -> List[Path]:
    ...
    for dirpath, dirnames, filenames in os.walk(base, followlinks=False):
        ...
        for fname in filenames:
            ...
            if fpath.is_symlink():
                continue
            if _should_exclude(fpath.relative_to(base)):
                continue
            files.append(fpath)
    return files
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    return bool(_policy_attr("should_exclude")(rel_path))
...
def _is_sqlite_file(path: Path) -> bool:
    ...
    return bool(_policy_attr("is_sqlite_file")(path))
...
def _safe_copy_db(
    ...
    *,
    ...
    """
    policy = _load_backup_policy_module()
    # Keep legacy monkeypatch-based tests targeting hermes_cli.backup working
    # while the policy itself lives in the external scripts owner.
    setattr(policy, "sqlite3", sqlite3)
    setattr(policy, "time", time)
    ok = bool(policy.safe_copy_db(src, dst, timeout_seconds=timeout_seconds))
    if not ok:
        logger.warning("SQLite safe copy failed for %s", src)
    return ok
