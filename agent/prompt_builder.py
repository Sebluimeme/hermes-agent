...
KANBAN_GUIDANCE = (
    ...
    "task). The compact response includes the task, recent comments/events, "
    "run evidence and continuation cursors. Use `detail='full'` only when the "
    "compact view says older context was omitted and you genuinely need it; "
    "do not repeat an unchanged full read.\n"
    "2. **Work inside the workspace.** `cd $HERMES_KANBAN_WORKSPACE` before "
    "any file operations. Don't modify files outside it unless asked. Follow "
    "the card's verification and delivery contract: use targeted checks first; "
    "run full suites, builds, visual or production checks only when contracted. "
    "Reuse an exact PASS while commit and workspace fingerprint are unchanged; "
    ...
    "5. **Preflight every finish.** After running the relevant verification, "
    "call `kanban_completion_ready(summary=..., metadata=..., artifacts=[...])` "
    "with exactly the evidence you intend to submit. If it returns `ready=true`, "
    "call `kanban_complete()` immediately: the runtime reuses that exact validated "
    "handoff even if you omit its fields, so do not read files or run another check. "
    "If it returns `ready=false`, supply the missing proof or block with the exact "
    "reason; do not burn a failed `kanban_complete` call. Then finish with the review model encoded "
    "by the task graph. Always "
    ...
    "choice would strand or duplicate the downstream lane. A bounded `express` "
    "task completes after its targeted check; it does not request a reviewer. "
    "Only when this card explicitly requires an independent review, call "
    ...
    "not trip unblock-loop detection. Visual review is opt-in. Only a contract "
    "with `visual_review_required=true` or `[VISUAL]` adds absolute desktop and "
    "mobile paths under `metadata.visual_review.screenshots`; one GPT/Coder "
    "reviewer decides. Never call or wait for Gemini.\n"
    ...
    "## Orchestrator mode\n"
    "\n"
    "For a decomposition task, create one child per specialist with explicit "
    "`assignee` and `parents=[...]`, then complete your routing task; do not "
    "implement the children yourself.\n"
    "A board-only control task (triage/supervision) must omit workspace fields "
    "to get private scratch. Never attach such a task to a project root: its "
    "broad Git lock would idle unrelated lanes.\n"
    "\n"
    ...
    "- **Replacement cards.** When a new card takes over a blocked/triage "
    "card, create it with `resolves=[<old-task-id>]`. On successful completion "
    "the kernel archives the old card and clears its obsolete human action; "
    "never describe a replacement only in prose.\n"
