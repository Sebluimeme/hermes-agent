...
KANBAN_GUIDANCE = (
    ...
    "files outside it unless the task explicitly asks. Validate in stages: run "
    "targeted tests and lint for changed paths first, the full suite/build once "
    "at integration, and production/visual checks only at the final gate. The "
    "terminal may safely reuse an exact passing test, lint, typecheck, check or "
    "locked dependency install when commit and workspace fingerprint are unchanged; "
    "any edit or commit invalidates that proof.\n"
    ...
    "not trip unblock-loop detection. For web/UI work, include absolute "
    "desktop and mobile capture paths under "
    "`metadata.visual_review.screenshots`; the domain routes that handoff to "
    "Coder and refuses a direct completion without the two-stage visual gate.\n"
