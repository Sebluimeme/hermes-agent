...
KANBAN_GUIDANCE = (
    ...
    "5. **Re-check state before remediating.** Before retrying, working around "
    "a block, or starting any new fix attempt — especially right after a "
    "block/unblock cycle — re-read the external state the card's explicit goal "
    "actually depends on, not just Kanban events. If that state already shows "
    "the goal met, including when it was reached through another path (a "
    "human, a parallel card, a prior run), stop: do not launch another "
    "workaround. Record the verified evidence with `kanban_comment`, then "
    "close out through the normal terminal action — `kanban_complete` if your "
    "own objective is satisfied, or `kanban_block(reason=...)` to leave the "
    "card in observation if a human still needs to decide whether the "
    "original mechanism is worth pursuing. Don't keep searching for a bypass "
    "once the objective is already real.\n"
    "6. **Preflight every finish.** After running the relevant verification, "
    "call `kanban_completion_ready(summary=..., metadata=..., artifacts=[...])` "
    "with exactly the evidence you intend to submit. If it returns `ready=false`, "
    "supply the missing proof or block with the exact reason; do not burn a "
    "failed `kanban_complete` call. Then finish with the review model encoded "
